...
// activity variables
var activityDurations = [1,5,10,15,20,30,45, 60, 75, 90, 105, 120, 180, 240, 300, 360, 420, 480];
...
function saveAddActivity(streamonly){
    ...
    if(!newactpinMoved){
	var answer = confirm("It seems like you didn't set the location pin on the map. Are you sure you want to continue?");
	if (answer){
	}
	else{
	    return;
...
function updateSysStream(){
    ...
    //
    //  Add constraints based on what needs revision.... tid specific (a hack for checking the revision feature)
    // 
    // 
    if(tid =='92f792d5f805b79974466450f528d24e'){
//	generalConstraint("Help us improve mobi.", "Please share your thoughts; all comments are welcomed.");
//	generalConstraint("Get in touch with me.", "My email is hq@eecs.harvard.edu");
//	generalConstraint("Please ask a question.", "We'd appreciate some questions, currently no questions has been asked.");

	generalConstraint("The day is too packed.", "The harbor island suggestions are great but one island would be enough. Please adjust time durations accordingly so the day is not so packed.");
	generalConstraint("The dinner times don't make sense.", "Please make sure we are having dinner at a reasonable hour.");
    }

    if(tid == 'a31b5015bac8dce3a4e417b5d7fdcb31'){
	generalConstraint("Let's just stay midtown and remove downtown activities.", "I think it may be too much to go uptown and downtown all in one day, so let's just skip downtown and stay mid/uptown.");
    }
...
//    alert(endTime);
	...
	var n = new note("Spend more time on existing activities or add more activities to the itinerary", 
			 problem, 
			 ['todo', 'time']);
	si = new streamitem('todo', n , null);
	si.id = 'sys_have_time_left';
    }else if(freetime < -1 * allowedOver){ // no time left
	var problem = 'Try removing, editing, or reordering activities in the itinerary to save time. The trip should end by ' + minToTime(endTime) + '.';
