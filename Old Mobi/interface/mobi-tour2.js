...
// activity variables
var activityDurations = [1,5,10,15,20,30,45, 60, 75, 90, 105, 120]; // in minutes
var campuslocations = [];
var lastSearch = null;
...
function GetMap()
{
    ...
    map.LoadMap(new VELatLong(42.37355775, -71.1188900), defaultZoom, 'r', false, VEMapMode.Mode2D, true, 0, mapOptions);
    map.HideScalebar();
...
// todo: should really choose a random direction
//	    alert("too close");
	    ll = new VELatLong(parseFloat(si.data.location.lat) + tolerance, parseFloat(si.data.location.long) + tolerance);
	}
    }
...
function createStreamItem(si){
    ...
    descntag.append('<br/>');
    for(var i = 0; i < si.data.categories.length; i++){
	descntag.append("<a class='tagstreamitem' href='#' onclick=" + "\"filterOnTag('" + si.data.categories[i] + "')\">" + '#' + si.data.categories[i] + "</a>");
    }
    msg.append(descntag);
