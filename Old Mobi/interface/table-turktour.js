...
// activity variables
var activityDurations = [1,5,10,15,20,30,45, 60, 75, 90, 105, 120, 180, 240];
...
var frames = [];
var changes = [];
var 	    submitFramesAlready = false;

function frame(){//type, change, turkerId, assignmentId, identifier, constraintStat, bars, tripTime, totalTime){
	this.constraintStat = [];
//	frames[frames.length - 1].timeStat = [];
    this.bars = [];
    this.times = [];
    // this.type = type;
    // this.change = change;
    ...
    // this.constraintStat = constraintStat;
    // this.bars = bars;
...
function GetRoute(locations){
    if(tid == '1809173cf376c67bee886a883a18511a'){
	GetRouteOld(locations);
    }
...
//	shape.SetLineWidth(2);
	...
	if(typeof(frames[frames.length-1]) != 'undefined'){  
	    frames[frames.length-1].times[i] = new Object();
	    frames[frames.length-1].times[i].start = time;
	    frames[frames.length-1].times[i].end = next;
	    frames[frames.length-1].times[i].categories = getItem(itinerary[i]).data.categories;
	}
    ...
    if(typeof(frames[frames.length-1]) != 'undefined'){
	frames[frames.length - 1].tripTime = actualend - beginTime;
	frames[frames.length - 1].beginTime = beginTime;
	frames[frames.length - 1].endTime = endTime;
	frames[frames.length - 1].actualEnd = actualend;
	frames[frames.length - 1].allowedTime = endTime - beginTime;
    }
...
function printCat(cat){
    if(cat =='casino poker rooms (low stakes)'){
	return 'low-stakes poker';
    }
    if(cat =='break'){
	return 'break';
    }
...
function createStreamItem(si){
    ...
    msg.append('<br/>');
    var desc = takeTill(si.data.description, 120);
    ...
    if(debug){
	if(si.type != 'todo'){
	    descntag.append("<br/>" + displayTime(si.createTime) + "<br/>Worker: " + si.userId + "<br/>HIT: " +  si.assignmentId +"");
	}
    }
...
function takeTill(str, maxchars){
	...
	if(sp[i].indexOf('tour_main.asp') != -1){
	    sp[i] = ' [...], ';
	    continue;
	}
...
function loadNextStreamItem(){
//    alert(JSON.stringify(allStreams[userStream.length]));
    if(userStream.length < allStreams.length){
	frames[frames.length - 1].turkerId = allStreams[userStream.length].userId;
	frames[frames.length - 1].assignmentId =  allStreams[userStream.length].assignmentId;
	frames[frames.length - 1].identifier =  allStreams[userStream.length].number;
	frames[frames.length - 1].createdBy = allStreams[userStream.length].createBy;
	frames[frames.length - 1].isNew = allStreams[userStream.length].isNew;


	if(typeof(allStreams[userStream.length].strChange) != 'undefined'){
	    frames[frames.length - 1].strChange = allStreams[userStream.length].strChange;
//	    alert(allStreams[userStream.length].number + ", " + allStreams[userStream.length].createBy + ", " + allStreams[userStream.length].userId + "," + allStreams[userStream.length].isNew + ", " + allStreams[userStream.length].strChange);
	}
	userStream.push(allStreams[userStream.length]);
	
	loadStateIntoInterface();
    }
}

function loadPreviousStreamItem(){
   ...
   jQuery.ajax({
	...
	success: function(obj){
		...
		for(var i = 0; i < obj.length; i++){
		    var streamItem = eval('(' + obj[i].answer + ')');
		    streamItem.id = 'user_' + obj[i].hitId;
		    streamItem.userId = obj[i].userId;
		    streamItem.submitTime = obj[i].submitTime;
		    streamItem.assignmentId = obj[i].assignmentId;
		    streamItem.changeInfo = obj[i].changeInfo;
		    streamItem.number = obj[i].hitId;

		    if(typeof(notOriginal[obj[i].hitId]) == 'undefined'){
		    	// original!
		    	streamItem.isNew = true; // this is an original contribution
		     	original[obj[i].hitId] = streamItem.userId;
		    	streamItem.createBy = streamItem.userId;
		    }else{
		     	streamItem.isNew = false; // this is an edit
		     	streamItem.createBy = original[obj[i].hitId];
			streamItem.strChange = howChanged(streamItem, allStreams[notOriginal[obj[i].hitId]]);
		    }

		    if(streamItem.changeInfo != "NULL"){
		    // 	// this guy gets editted later by that number
			notOriginal[streamItem.changeInfo] = i;
		     	original[obj[i].changeInfo] = original[obj[i].hitId];
		     }

		    userStream = [];
		    allStreams.push(streamItem);
		    // userStream.push(eval('(' + obj[i].answer + ')'));
		    // userStream[count].id = 'user_' + obj[i].hitId;
		    // userStream[count].userId = obj[i].userId;
		    // userStream[count].submitTime = obj[i].submitTime;
		    //		    count++;
	
		}
		allStreams.sort(function(a, b) { return mysqlTimeStampToDate(a.submitTime) - mysqlTimeStampToDate(b.submitTime);});
	    }
	}
    });
    return;
...
//     }
	...
	currentState++;
//	if(currentState == '106'){
//	    alert(userStream.length);
//	}
	state = allstate[currentState];
	frames[frames.length - 1].turkerId = state.userId;
	frames[frames.length - 1].assignmentId =  state.assignmentId;
	frames[frames.length - 1].stateIdentifier =  state.stateId;
	frames[frames.length - 1].itchange = changes[currentState];
	stateId = state.stateId;
        state = eval('(' + state.state + ')');
	loadHostData(state.admin); // requester's stuff
	loadStateIntoInterface();
...
function writeFrames(){
    ...
    if(!submitFramesAlready){
    jQuery.ajax({
	type: "POST",
	url: "http://people.csail.mit.edu/hqz/mobi/submitFrames.php",
//	async: false,
	data: ({
	    answer: escape(JSON.stringify(frames)),
	    taskId: tid, 
	    type: "turktour"}),
	success: function(msg){
	    alert(msg);
	    submitFramesAlready = true;
	   // alert("frames saved to server");
	}});
    }
}
...
function howChanged(a, b){
    // coordinates can change
    if((a.data.location.lat != b.data.location.lat) || 
       (a.data.location.long != b.data.location.long)){
	return "location";
    }
    // duration can change
    ...
    if(a.data.categories.length == b.data.categories.length){
	for(var i = 0; i< a.data.categories.length; i++){
	    if(a.data.categories[i] != b.data.categories[i]){
		return "categories";
	    }
	}
    }
    // name/description can change
    if((a.data.name != b.data.name) || (a.data.description != b.data.description)){
...
function loadAllState(){
   jQuery.ajax({
      type: "GET",
      // dataType: "json", 
      url: "http://people.csail.mit.edu/hqz/mobi/loadTurkTourAllState.php",
      data: ({type: "tour", id: tid}),
       async: false, 
      success: function(obj){
	  if(obj == ""){
	  }else{
	      allstate =  eval('(' + obj + ')'); // array of states
	      var its = [];

	      changes.push(null);
	      for(var i = 0; i < allstate.length; i++){
		  its.push(eval('(' + allstate[i].state + ')').itinerary);
		  if(its.length > 1){
	              changes.push(whatChanged(its[i], its[i - 1]));
		  }
	      }
	      state = allstate[currentState];
	      stateId = state.stateId;
...
function problemStatement(constraint, predData){
	...
	statement = 'We need ' + constraint.compare + ' ' + readMinutes(Math.round(constraint.value*60)) + ' of ' + printCat(constraint.cat) + '.' + ' The current itinerary contains ';
	if(value == 'no'){
	    statement += "no " + printCat(constraint.cat) + '.';
	}else{
	    statement += readMinutes(Math.round(predData.value * 60)) + " of " + printCat(constraint.cat) + ". ";
	}
    }else {
	statement = 'We need ' + constraint.compare + ' ' + constraint.value + ' ' + printCat(constraint.cat) + ' ' + unit + '.' +  ' The current itinerary contains ' + value + ' ' + printCat(constraint.cat) + ' activities. ';
    }

    if(value != 'no'){
	statement += "<br/><br/>The following " + printCat(constraint.cat) + " activities are in the current itinerary: <br/>";
...
function updateSysStream(){
	...
	if(typeof(frames[frames.length-1]) != 'undefined'){
	    frames[frames.length - 1].constraintStat[i] = predData.sat;
	}
    ...
    if(sysStream.length == 0){
	generalConstraint("Continue to improve the itinerary.", "Make sure the itinerary is coherent. If you see an issue, please fix it or add a note on what needs work.");
    }
...
function loadStateIntoInterface(){
    ...
    if(debug){
	$('#stateInfo').html("State: " + currentState + "<br/>Time: " + displayTime(mysqlTimeStampToDate(allstate[currentState].submitTime)) + "<br/>userId: " + allstate[currentState].userId);
    }
...
//	$('#stime_' + si.id).append(createInItineraryButton(si));
    ...
    // do tag row
    for(var i = 0; i < categories.length; i++){
	tag = "<span style='white-space:nowrap;'><a class='tagrowitem' href='#' onclick=" + "\"filterOnTag('" + categories[i] + "')\">" + '#' + printCat(categories[i]) + "</a></span>";
	$('#tagrow').append(tag);// + '&nbsp;&nbsp;');
    }
...
//    state.admin.constraints[1].value = 1;
	...
	success: function(msg){
	    if(msg == ""){
		// cannot save it
		alert("It appears that someone else has recently made a change that conflicts with the changes you are trying to save. If you have already contributed another idea in this HIT, you can submit below. Otherwise, please refresh the page before continuing. We apologize for the inconvenience.");
		ret = false;
		return;
	    }else {
		madeChange = true;
...
function disableItSave(){
    ...
    $('#saveitbutton').text('drag activities to reorder, click to edit/remove');
...
function enableItSave(){
	...
	$('#saveitbutton').text('submit HIT with itinerary changes!');
...
function loadHostData(data){
    ...
    if(tid == '7ce5948bb8e05d7a94d58e51f5050a24'){
	for(var i = 0; i < constraints.length; i++){
	    if(constraints[i].compare == "at most"){
		constraints[i].compare = "at least";
	    }
	}
    }
    start = data.start;
    ...
    if(data.creator != undefined && data.creator == 'itonly'){
...
function generalConstraint(title, msg){
    var n = new note(title, 
		     msg,
			 ['todo']);
...
//    alert(endTime);
    ...
    if(typeof(frames[frames.length-1]) != 'undefined'){  
	frames[frames.length-1].timeStat = freetime /  (endTime - beginTime);
    }
...
function displaySingleConstraint(i){
    ...
    if(unit == 'hours'){
	if(v == 0.5){
   str = 'spend ' + compare + ' 30 minutes ' + 'on ' + "<u>" + cat + "</u>.";
	}else 	if(v == 1){
...
function predicateResponse(response, value, activities, explain, sat){
...
function generatePredicate(constraintDesc){
	...
	if(compare == 'at most'){
		...
		return new predicateResponse(total <= v, total, activities, "Too much time on '" + printCat(cat) + "' in the itinerary", sat);
	    }else if(unit=='activities'){
		
		return new predicateResponse(total <= v, total, activities, "Have too many '" + printCat(cat) + "' activities in the itinerary", sat);
	    }
	    ...
	    if(unit=='hours'){
		return new predicateResponse(total >= v, total, activities, "Add more '" + printCat(cat) + "' to the itinerary", sat);
	    }else if(unit=='activities'){
		return new predicateResponse(total >= v, total, activities, "Add more '" + printCat(cat) + "' to the itinerary", sat);
	    }
	    ...
	    if(unit=='hours'){
		if(total > v){
		    explain = "Too much time on '" + printCat(cat) + "' in the itinerary";
		}else if(total < v){
		    explain = "Add more '" + printCat(cat) + "' to the itinerary";
		}
		return new predicateResponse(total == v, total, activities, explain, sat);
	    }else if(unit=='activities'){
		if(total > v){
		    explain = "Have too many '" + printCat(cat) + "' activities in the itinerary";
		}else if(total < v){
		    explain = "Add more '" + printCat(cat) + "' to the itinerary";
		}
		return new predicateResponse(total == v, total, activities, explain, sat);
...
//	if(params.assignmentId == "AEWOJD2MZRT79"){
//	    alert("You have reached your HIT quota for the day. As a precaution, the requester will manually check your submissions prior to accepting your work.");
// Please note that while the HIT pays for each additional contribution, the requester will check to see whether you have actually contributions on each edit or whether
...
function figureItOut(event){
	if(event.keyCode == 78){
	    if(userStream.length < allStreams.length){
//		alert("Idea time: " + allStreams[userStream.length].submitTime + "\nState time:" + allstate[currentState+1].submitTime);
		if(currentState >= allstate.length - 1 || Math.abs(mysqlTimeStampToDate(allstate[currentState+1].submitTime) - mysqlTimeStampToDate(allStreams[userStream.length].submitTime)) < 1000){
		    frameId+=1;
		    frames.push(new frame());
		    frames[frames.length-1].type = "both";
		    loadNextStreamItem();

		    loadNextState();
//		    alert(JSON.stringify(frames));
		}else if(currentState >= allstate.length - 1 || mysqlTimeStampToDate(allstate[currentState+1].submitTime) > mysqlTimeStampToDate(allStreams[userStream.length].submitTime)){
		    frameId+=1;		
		    frames.push(new frame());
		    frames[frames.length-1].type = "str";
		    loadNextStreamItem();
...
//		    alert(JSON.stringify(frames));
		    ...
		    frameId+=1;
		    frames.push(new frame());
		    frames[frames.length-1].type = "it";
		    loadNextState();
...
//		    alert(JSON.stringify(frames));
	  ...
	  //  event.preventDefault();
	    ...
	    if(userStream.length > 0){
		if(currentState == 0 || mysqlTimeStampToDate(userStream[userStream.length - 1].submitTime) > mysqlTimeStampToDate(allstate[currentState].submitTime)){
		    loadPreviousStreamItem();
		}else{
		    loadPreviousState();
		}
	    }else{
