...
$server = "http://people.csail.mit.edu/hqz/mobi/mobi-friends.html";
...
while($row = mysql_fetch_array($results, MYSQL_ASSOC)) {
    ...
    $to = $email;
    $subject = "Your Mobi link for helping $requestername";
    $userId = $row['userId'];
    echo $userId;
    $name = explode(" ", $row['name']);
    $name = $name[0];

    $body = "Hi $name,\n\nYour personal link for helping $requestername plan a day of their trip is:\n\n $server?tid=$taskId&uid=$userId\n\n Best,\nMobi admin";
    ...
    $html = file_get_contents("subjectfriendemail.html");
    ...
    $html = str_replace("___LOCATION___", $location, $html);
    $html = str_replace("___REQUESTER___", $requestername, $html);
