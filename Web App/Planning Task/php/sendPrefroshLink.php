...
while($row = mysql_fetch_array($results, MYSQL_ASSOC)) {
    ...
    $from = "Mobi Event Planning <hq@eecs.harvard.edu>";
    $to = $email;
    $subject = "Your planning link for the ultimate prefrosh weekend";
    $userId = $row['userId'];
    $name = explode(" ", $row['name']);
    $name = $name[0];

    $body = "Hi $name,\n\nYour personal link for planning the ultimate pre-frosh weekend with other students is:\n\n $server?tid=$taskId&uid=$userId\n\n Best,\nmobi tour admin";
    ...
    $html = file_get_contents("prefroshemail.html");
