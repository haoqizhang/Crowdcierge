...
function createStreamItem(si){
    ...
    if(debug){
	if(si.type != 'todo'){
	    descntag.append("<br/>" + displayTime(si.createTime) + "<br/>Idea from: " + si.userId);
	}
    }
...
function loadAllState(){
   jQuery.ajax({
      type: "GET",
      // dataType: "json", 
      url: "http://people.csail.mit.edu/hqz/mobi/loadTourAllState.php",
      data: ({type: "tour", id: tid}),
       async: false, 
      success: function(obj){
	  if(obj == ""){
...
function loadStateIntoInterface(){
	...
	var si  = getItem(itinerary[i]);
	if(si == null){
//	    si = allStreams.filter(function(x){
//		return x.id == itinerary[i];
//	    })[0];
//	    alert(itinerary);
