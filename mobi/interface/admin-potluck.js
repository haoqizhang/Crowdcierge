...
// load the menu by catergory
	...
	for(var k = 0; k < categories.length; k++){
	    menu += "<div id='menuCategory" + k + "' class='style_1'><b>" + categories[k] + "</b><br/><div id='menuContent" + k +  "'>";	    
	    
	    for(var i = 0; i < state.users.length; i++){
		for(var j = 0; j < state.users[i].choices.length; j++){
		    if(state.users[i].choices[j].type == categories[k]){
			if(state.users[i].choices[j].selected){
			    menu += "<div id='menu" + state.users[i].userId + state.users[i].choices[j].id + "' class='paragraph_style'><span style='color:rgb(0,0,0);line-height: 18px;margin-left:5px;font-size:0.75em' class='style_1'>" + state.users[i].choices[j].description + " (" + shortName(state.users[i].name) + ")<br/></span></div>";
			}else{
			    menu += "<div id='menu" + state.users[i].userId + state.users[i].choices[j].id + "' class='paragraph_style' style='display:none'><span style='color:rgb(0,0,0);line-height: 18px;margin-left:5px;font-size:0.75em' class='style_1'>" + state.users[i].choices[j].description + " (" + shortName(state.users[i].name) + ")<br/></span></div>";
   ...
   // load resources
	...
	for(var j = 0; j < state.users[i].choices.length; j++){
		...
		if(!hasItems){// first time
		    hasItems = true;
		    anyItems = true;
   row.append("/ <a href='#' class='style_1' style='color:#0000CE' onclick=\"request(" + i + "," + j + ")\"><u>" + state.users[i].choices[j].description  + "</u></a>" + " / ");
	...
	var color = Math.floor((i / (state.preferenceOrdering.length)) * 125)
 	...
 	for(var j = 0; j < pl.length; j++){
		...
		var preferenceInfo = pl[j].description + " (" + 
 		    shortName(state.users[userKeys[state.preferenceOrdering[i].userId]].name) + ")<br/>";
		// 		peoplesPreferences += preferenceInfo;
		// add it to sortable
//		var e = "<input type=\"checkbox\" name=\"choiceSelected\"></input>";


		var stuff = "<li id='po" + i + "'><p class='paragraph_style'><span class='ui-icon ui-icon-arrowthick-2-n-s' style='width:12px'></span><span style='font-size:0.75em;display:block;line-height:18px;margin-left:15px;width:190px;color:rgb(" + color + "," + color + "," + color + ")' class='style_1'><span id='preference" + pl[j].id + "'>" + preferenceInfo + "</span></span></p>";
		jQuery('#sortable').append(stuff);


//		jQuery('#sortable').append("<li id='po" + i + "'><span class='ui-icon ui-icon-arrowthick-2-n-s'></span><span style='font-size:0.9em;line-height:18px;margin-left:10px;width:170px;color:rgb(" + color + "," + color + "," + color + ")' class='style_1'>" + preferenceInfo + "</span></li>");
...
function createChoiceField(field, val, selectedIndex, picked, textOnly){
    ...
    if (! ua.msie ) {
	jQuery(itemDiv).css('margin-left', '35px');
    }
    ...
    if(textOnly){
	c = "<span style='display:inline-block;width:230px'>" + val + "</span>";
	d = "<span style='margin-left:10px;display:inline-block;width:100px'>" + categories[selectedIndex] + "</span>";
    }else{

	c = "<span style='display:inline-block;width:230px'><input type=\"text\" maxlength='40' onkeyup='updateMenuItemDescription(this)' name=\"choiceSet\" value=\"" + val + "\"style='width:230px'\"></input></span>";
	d = "<span style='margin-left:10px;display:inline-block;width:100px'><select name=\"choiceCategory\" onchange='changeCategoryDisplay(this)' style='width:100px'></span>";

	var ka = jQuery.browser;
	if ( ka.msie ) {
	    c = "<span style='display:inline-block;width:210px;margin-right:0px;'><input type=\"text\" maxlength='40' onkeyup='updateMenuItemDescription(this)' name=\"choiceSet\" value=\"" + val + "\"style='width:210px'\"></input></span>";
	    d = "<span style='margin-left:30px;margin-right:10px;display:inline-block;width:100px'><select name=\"choiceCategory\" onchange='changeCategoryDisplay(this)' style='margin-left:30px;margin-right:10px;width:90px'></span>";
	}
    ...
    if(textOnly){
	if(picked){
	    e = "<input type=\"checkbox\" onclick='updateCheck(this)' style='vertical-align:middle;margin-left:12px' name=\"previousChoiceSelected\" checked></input>";
	}else{
	    e = "<input type=\"checkbox\" onclick='updateCheck(this)' style='vertical-align:middle;margin-left:12px' name=\"previousChoiceSelected\"></input>";
	}
    }else{
	var e;
	if(picked){
	    e = "<input type=\"checkbox\" onclick='updateCheck(this)' style='vertical-align:middle;margin-left:12px' name=\"choiceSelected\" checked></input>";
	}else{
	    e = "<input type=\"checkbox\" onclick='updateCheck(this)' style='vertical-align:middle;margin-left:12px' name=\"choiceSelected\"></input>";
	}
    }
    var f = "<img src='exit.png' width='11' style='vertical-align:middle;margin-left:40px' onClick='removeItem(this)'>";
...
function updatePreferenceDescription(e){
	...
	jQuery('#newpref' + newPreferences[index].id).css('display', 'block');
    }
}
...
function createPreferenceDisplay(id, desc){
    var color = 0;
    var stuff = "<li style='display:none' id='newpref" + id + "'><p class='paragraph_style'><span class='ui-icon ui-icon-arrowthick-2-n-s'></span><span style='font-size:0.75em;display:block;line-height:18px;margin-left:15px;width:190px;color:rgb(" + color + "," + color + "," + color + ")' class='style_1'><span id='preference" + id + "'><span id='newprefdescription" + id + "'>" + desc + "</span> (" + shortName(username) + ")</span></span></span></p></li>";
    jQuery('#sortable').prepend(stuff);
}

function createMenuItemDisplay(id, catIndex, desc){
    var item = "<div id='newmenu" + id + "' class='paragraph_style' style='width:200px;display:none'> <span style='color:rgb(0,0,0);line-height: 18px;margin-left:5px;font-size:0.75em' class='style_1'><span id='newdescription" + id + "'>" + desc + "</span><span> (" + shortName(username) + ")</span></span> </div>";
...
function updateMenuItemDescription(e){
	...
	jQuery('#newmenu' + newChoices[index].id).css('display', 'block');
    }
}
...
function createField(field, name, val, size, textOnly){
    ...
    if(textOnly){
	c = "<span style='display:inline-block;width:" + size + "px'>" + val + "</span>";
    }else{
	c = "<span style='display:inline-block;width:" + size + "px'><input type=\"text\" maxlength='80' onkeyup='updatePreferenceDescription(this)' name=\"" + name + "\" value=\"" + val +  "\" style='width:" + size + "px'\"/></span>";
	
    }
    var fname = '#' + field;
    var f = "<img src='exit.png' width='11' style='vertical-align:middle;margin-left:25px' onClick='removePreference(this)'/>";
...
function showMobi(){
    jQuery('#mobi-content').css('display', 'block');
}
