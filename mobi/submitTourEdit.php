...
if(strcmp("tour", $type) == 0){
  ...
  $check = "SELECT * FROM tour_$taskId where hitId=$oldid and changeInfo IS NOT NULL";
  ...
  $addquery = "INSERT INTO tour_$taskId (userId, answer, submitTime) VALUES ('$userId', '$answer', now())";
  ...
  $query = "UPDATE tour_$taskId SET changeInfo = $newid WHERE hitId=$oldid";
...
if(mysql_num_rows($results) == 0){
  ...
  while($row = mysql_fetch_array($results, MYSQL_ASSOC)) {
    ...
    if($changed){
      ...
      $newquery = "INSERT INTO tour_state_$taskId (state, userId, submitTime, changeInfo) VALUES ('$newstate', '$userId', now(), '$oldidstr')";
