...
while($row = mysql_fetch_array($results, MYSQL_ASSOC)) {
    ...
    $to = $email;
    $subject = "Your planning link for an outing to Boston";
    $userId = $row['userId'];
    $name = explode(" ", $row['name']);
    $name = $name[0];

    $body = "Hi $name,\n\nYour personal link for planning a fun Boston outing with other students is:\n\n $server?tid=$taskId&uid=$userId\n\n Best,\nmobi tour admin";
    ...
    $html = file_get_contents("bostonemail.html");
