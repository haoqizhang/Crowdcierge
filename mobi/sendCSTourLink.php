...
while($row = mysql_fetch_array($results, MYSQL_ASSOC)) {
    ...
    $to = $email;
    $subject = "Your group outing planning link";
    $userId = $row['userId'];
    $name = explode(" ", $row['name']);
    $name = $name[0];

    $body = "Hi $name,\n\nYour personal link for planning the group outing with others in your group is:\n\n $server?tid=$taskId&uid=$userId\n\n Best,\nmobi admin";
    ...
    $html = file_get_contents("csemail.html");
